...
class SettingsActivity : AppCompatActivity() {
    ...
    private fun restoreFromLastBackup() {
        Toast.makeText(this, "Restore from last backup is not supported in this version.", Toast.LENGTH_SHORT).show()
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            REQUEST_BACKUP -> {
                data.data?.let { uri ->
                    lifecycleScope.launch {
                        val success = transactionRepository.backupToUri(this@SettingsActivity, uri)
                        if (success) {
                ...
                data.data?.let { uri ->
                    AlertDialog.Builder(this)
                        ...
                        .setPositiveButton("Yes") { _, _ ->
                            lifecycleScope.launch {
                                val success = transactionRepository.restoreFromUri(this@SettingsActivity, uri)
                                if (success) {
